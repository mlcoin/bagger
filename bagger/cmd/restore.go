...
var restoreCmd = &cobra.Command{
	Use:   "restore",
	Short: "Restore Bagger database.",
	Long: `Restore Bagger database from a file.
...
func init() {
	...
	restoreCmd.Flags().StringVarP(&restoreFile, "backup-file", "f",
		"bagger.bak", "File to restore from")
}

func doRestore(cmd *cobra.Command, args []string) error {
	// Check if the DB already exists
	manifestFile := path.Join(sstDir, bagger.ManifestFilename)
