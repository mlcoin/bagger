...
var testCmd = &cobra.Command{
	Use:   "bank",
	Short: "Run bank test on Bagger.",
	Long: `
This command runs bank test on Bagger, inspired by Jepsen. It creates many
...
var bankTest = &cobra.Command{
	Use:   "test",
	Short: "Execute bank test on Bagger.",
	RunE:  runTest,
}
...
var bankDisect = &cobra.Command{
	...
	Long: `
Disect the bank output BaggerDB to find the first transaction which causes
...
func getBalance(txn *bagger.Txn, account int) (uint64, error) {
...
func putBalance(txn *bagger.Txn, account int, bal uint64) error {
...
func moveMoney(db *bagger.DB, from, to int) error {
...
// func iterateTotal(db *bagger.DB) ([]account, error) {
// 	expected := uint64(numAccounts) * uint64(initialBal)
// 	var accounts []account
// 	err := db.View(func(txn *bagger.Txn) error {
// 		// start := time.Now()
// 		itr := txn.NewIterator(bagger.DefaultIteratorOptions)
// 		defer itr.Close()
...
func seekTotal(txn *bagger.Txn) ([]account, error) {
	expected := uint64(numAccounts) * uint64(initialBal)
	var accounts []account
...
func findFirstInvalidTxn(db *bagger.DB, lowTs, highTs uint64) uint64 {
	...
	err := checkAt(midTs)
	if err == bagger.ErrKeyNotFound || err == nil {
...
func compareTwo(db *bagger.DB, before, after uint64) {
...
func runDisect(cmd *cobra.Command, args []string) error {
	...
	db, err := bagger.OpenManaged(opts)
	if err != nil {
		return err
	}
	...
	var min, max uint64 = math.MaxUint64, 0
	{
		...
		iopt := bagger.DefaultIteratorOptions
