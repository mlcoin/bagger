...
var infoCmd = &cobra.Command{
	Use:   "info",
	Short: "Health info about Bagger database.",
	Long: `
This command prints information about the bagger key-value store.  It reads MANIFEST and prints its
info. It also prints info about missing/extra files, and general information about the value log
files (which are not referenced by the manifest).  Use this tool to report any issues about Bagger
to the Dgraph team.
`,
...
func tableInfo(dir, valueDir string) error {
	...
	for _, t := range tables {
		lk, lv := bkey.ParseKey(t.Left), bkey.ParseVersion(t.Left)
		rk, rv := bkey.ParseKey(t.Right), bkey.ParseVersion(t.Right)
...
func printInfo(dir, valueDir string) error {
	...
	fp, err := os.Open(filepath.Join(dir, bagger.ManifestFilename))
	if err != nil {
		return err
	}
	defer func() {
	...
	manifest, truncOffset, err := bagger.ReplayManifestFile(fp)
	if err != nil {
		return err
	}
	fp.Close()
	...
	manifestInfo, ok := fileinfoByName[bagger.ManifestFilename]
	if ok {
		fileinfoMarked[bagger.ManifestFilename] = true
		truncatedString := ""
