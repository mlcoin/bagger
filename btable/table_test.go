...
func TestTableIterator(t *testing.T) {
	for _, n := range []int{99, 100, 101} {
		t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
			...
			table, err := OpenTable(f, boptions.MemoryMap)
			require.NoError(t, err)
			defer table.DecrRef()
...
func TestIterateFromEnd(t *testing.T) {
	...
	for _, n := range []int{99, 100, 101, 199, 200, 250, 9999, 10000} {
		t.Run(fmt.Sprintf("n=%d", n), func(t *testing.T) {
			f := buildTestTable(t, "key", n)
			table, err := OpenTable(f, boptions.FileIO)
			require.NoError(t, err)
			defer table.DecrRef()
			ti := table.NewIterator(false)
			defer ti.Close()
...
func TestConcatIterator(t *testing.T) {
	...
	require.NoError(t, err)
	defer tbl.DecrRef()
	tbl2, err := OpenTable(f2, boptions.LoadToRAM)
	require.NoError(t, err)
	defer tbl2.DecrRef()
	tbl3, err := OpenTable(f3, boptions.LoadToRAM)
	require.NoError(t, err)
	defer tbl3.DecrRef()
...
func TestMergingIterator(t *testing.T) {
	...
	tbl1, err := OpenTable(f1, boptions.LoadToRAM)
	require.NoError(t, err)
	defer tbl1.DecrRef()
	tbl2, err := OpenTable(f2, boptions.LoadToRAM)
	require.NoError(t, err)
	...
	it := butils.NewMergeIterator([]butils.Iterator{it1, it2}, false)
	defer it.Close()
...
func TestMergingIteratorReversed(t *testing.T) {
	...
	it := butils.NewMergeIterator([]butils.Iterator{it1, it2}, true)
	defer it.Close()
...
func TestMergingIteratorTakeOne(t *testing.T) {
	...
	t1, err := OpenTable(f1, boptions.LoadToRAM)
	require.NoError(t, err)
	defer t1.DecrRef()
	t2, err := OpenTable(f2, boptions.LoadToRAM)
	require.NoError(t, err)
	defer t2.DecrRef()
...
func BenchmarkRead(b *testing.B) {
	...
	//	butils.Printf("Size of btable: %d\n", tbl.Size())
	b.ResetTimer()
	// Iterate b.N times over the entire btable.
	for i := 0; i < b.N; i++ {
		func() {
