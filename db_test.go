...
// Opens a bagger db and runs a a test on it.
func runBaggerTest(t *testing.T, opts *Options, test func(t *testing.T, db *DB)) {
	dir, err := ioutil.TempDir(".", "bagger-test")
	require.NoError(t, err)
...
func TestIterateParallel(t *testing.T) {
	...
	opt := DefaultOptions
	runBaggerTest(t, &opt, func(t *testing.T, db *DB) {
		var wg sync.WaitGroup
		var txns []*Txn
		for i := 0; i < N; i++ {
			wg.Add(1)
...
func TestPidFile(t *testing.T) {
	runBaggerTest(t, nil, func(t *testing.T, db *DB) {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "Another process is using this Bagger database")
	})
}
...
func TestInvalidKey(t *testing.T) {
	runBaggerTest(t, nil, func(t *testing.T, db *DB) {
		err := db.Update(func(txn *Txn) error {
			err := txn.Set([]byte("!bagger!head"), nil)
			require.Equal(t, ErrInvalidKey, err)

			err = txn.Set([]byte("!bagger!"), nil)
			require.Equal(t, ErrInvalidKey, err)

			err = txn.Set([]byte("!bagger"), []byte("BaggerDB"))
			require.NoError(t, err)
			return err
		})
		require.NoError(t, err)
		...
		require.NoError(t, db.View(func(txn *Txn) error {
			item, err := txn.Get([]byte("!bagger"))
			if err != nil {
				return err
			}
			require.NoError(t, item.Value(func(val []byte) error {
				require.Equal(t, []byte("BaggerDB"), val)
				return nil
			}))
			return nil
		}))
