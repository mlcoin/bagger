...
var (
	baggerPrefix = []byte("!bagger!")     // Prefix for internal keys used by bagger.
	head         = []byte("!bagger!head") // For storing value offset for replay.
	txnKey       = []byte("!bagger!txn")  // For indicating end of entries in txn.
	baggerMove   = []byte("!bagger!move") // For key-value pairs which got moved during GC.
)
...
func Open(opt Options) (db *DB, err error) {
	...
	db = &DB{
		...
		opt:           opt,
		manifest:      manifestFile,
		elog:          trace.NewEventLog("Bagger", "DB"),
...
// need to write "old versions after new versions" to the baggerMove keyspace. Thus, for normal
...
// batchSet applies a list of bagger.Entry. If a request level error occurs it
// will be returned.
...
// Sequence represents a Bagger sequence.
...
// MergeOperator represents a Bagger merge operator.
...
// Add records a value in Bagger which will eventually be merged by a background
